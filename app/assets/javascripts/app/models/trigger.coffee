class App.Trigger extends App.Model
  ...
  @configure_attributes = [
    ...
    { name: 'admin_folder_id',          display: __('Folder'),           tag: 'select',    multiple: false, null: true, relation: 'AdminFolder', display_full_name: true, nulloption: true, filter: App.AdminFolder.filterForTargetModel('Trigger') },
    ...
    { name: 'perform',                  display: __('Execute changes on objects'),      tag: 'ticket_perform_action', null: true, notification: true, trigger: true, subscribe: true, ai_agent: true },
